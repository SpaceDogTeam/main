...
void manualMoveOneLeg(int choiceLeg, int choiceDirA, int choiceDirB, int choiceSpeA, int choiceSpeB, int choiceDur){
  
  unsigned long tSign = millis();
  digitalWrite(motorEnablePinsArray[2*choiceLeg-2], choiceDirA);
  digitalWrite(motorEnablePinsArray[2*choiceLeg-1], choiceDirB);
  analogWrite(motorPwmPinsArray[2*choiceLeg-2], choiceSpeA);
  analogWrite(motorPwmPinsArray[2*choiceLeg-1], choiceSpeB);
  while(millis() < tSign + choiceDur);   //  ms tempo equal to choiceDur (duration)
  analogWrite(motorPwmPinsArray[2*choiceLeg-2], 0);
  analogWrite(motorPwmPinsArray[2*choiceLeg-1], 0);
...
void interpretData() {
  char  buff[30];
  sprintf(buff,"ack <%d,%d,%d,%d,%d,%d,%d>",flagTransmission,val1,val2,val3,val4,val5,val6);
  Serial.flush();
  Serial.print(buff);
  ...
  if (flagTransmission == 0){
    ...
    manualMoveOneLeg(val1,val2,val3,val4,val5,val6);   //leg, dirA (0 retract, 1 extend), dirB, speedA(0-255, 165 min to move), speedB, duration(ms)
  }
    ...
    reachAngle(val1,val2,val3,val4,val5);
  }
  else if (flagTransmission == 4){
    manualMoveTwoLeg(val1);
  }
