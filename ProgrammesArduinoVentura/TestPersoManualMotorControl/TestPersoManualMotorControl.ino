...
void reachAngle(double angle1, double angle2) {
  ...
  while ((delta1 > 2 or delta2 > 2) and millis() < tSign + 4000) {
    getPosition();
    delta1 = AJointSetpoint - AJointAng;
    delta2 = BJointSetpoint - BJointAng;
    delta1 = abs(delta1);
    delta2 = abs(delta2);
