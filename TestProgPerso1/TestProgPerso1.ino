...
uint16_t reachMargin = 0.5;  // angle margin when reaching new position (total margin is twice that amount)
...
void setup() {
  ...
  flagReachedAngleB = 1;
  */
  Serial.println("TroubleShooting111");
}
...
void reachAngle(double angle1, double angle2) {
  ...
  if (AJointSetpoint != AJointAng){
    flagReachedAngleA = 0;
  }
  else{
    flagReachedAngleA = 1;
  }
  if (BJointSetpoint != BJointAng){
    flagReachedAngleB = 0;
  }
  else{
    flagReachedAngleA = 1;
  ...
  while ((!flagReachedAngleA or !flagReachedAngleB) and millis() < tSign + 4000) {
    getPosition();
...
void interpretData() {
  ...
  if (flagTransmission == 0){
    if (val1 == 0 and val2 == 0){
      ...
      manualMove(val1,val2,val3,val4,val5);
  }
  else if (flagTransmission == 2){
    Serial.println("TroubleShooting222");
    getPosition();
